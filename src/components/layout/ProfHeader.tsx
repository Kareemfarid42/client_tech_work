// PROFESSIONALS DEDICATED HEADER - EDITING THIS WILL NOT AFFECT THE MAIN SITE
...
const navItems: NavItem[] = [
  { label: "Professionals Challenges", href: "#challenge" },
...
export const ProfHeader = () => {
  ...
  const handleSmoothScroll = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    ...
    const isProfPage = location.pathname.includes("professionals");
    if (!isProfPage) {
      navigate("/professionals.html" + href);
    } else {
          ...
          <nav className="hidden xl:flex items-center gap-1">
            {navItems.map((item) => {
                      ...
                      <div className="bg-neutral-950/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl py-2">
                        {item.dropdownItems.map((subItem) => (
                          <Link
                            key={subItem.label}
                            to={subItem.href}
                            className="block px-5 py-3 text-sm text-gray-300 hover:text-[#0D9488] hover:bg-white/5 transition-colors"
                          >
                            {subItem.label}
                          </Link>
                        ))}
              ...
              if (item.label === "Get Started") {
                return (
                  <ProfContactModal key={item.label}>
                      ...
                      <div className="flex flex-col gap-3 pl-4 border-l border-white/10 ml-1">
                        {item.dropdownItems.map((subItem) => (
                          <Link
                            key={subItem.label}
                            to={subItem.href}
                            className="text-sm font-medium text-white/80 hover:text-[#0D9488] transition-colors block"
